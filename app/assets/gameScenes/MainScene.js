...
function MainScene(window, game) {
	...
	var bomb_Sprite = null;					// Sprite of bomb
	var bomb_Moment = null;					// Moment attached to bomb sprite
	var bomb_Body = null;					// Body attached to bomb sprite	
	var bomb_Shape = null;					// Shape attached to bomb sprite
	
	var blocks_Sprite = [];					// Array that will contains the block sprites
	var blocks_Moment = [];					// Array that will contains the moments attached to block sprites
	var blocks_Shape = [];					// Array that will contains the shapes attached to block sprites
	var blocks_Body = [];					// Array that will contains the bodies attached to block sprites
	
	var shadow = null;						// Bomb shadow
	...
	var GROUND_LEVEL = 150;					// Floor height (Used in space, shadow and blocks creation)
	var bombLaunched = false;				// Detects if bomb was launched. Changes to true on TouchEnd and activate the enterframe
	var bombTouched = false;				// Detects if bomb was touched. If false, touchMove will not register the movement.
	var bombExploded = false;				// Detects if bomb has exploded
	...
	function createBackground(){
		...
		background = platino.createSprite({image:'graphics/background.png', width:cpX(0),height:cpY(0), z:0});
		self.add(background);
	...
	function createBomb(){
		...
		bomb_Sprite.setInitPosition({x:300,y:cpY(400)});
		...
		shadow = platino.createSpriteSheet({asset:Alloy.CFG.mainTexture, frame:4, z:1});
	...
	function gameLoop(e){
			...
			bomb_Sprite.center = {x:chipmunk.cpBodyGetPos(bomb_Body).x,y:cpY(chipmunk.cpBodyGetPos(bomb_Body).y)};
			...
			// Check http://docs.platino.io/#!/guide/chipmunk2d for more info:
				...
				while(i--){
					...
					if (!chipmunk.cpBodyIsSleeping(blocks_Body[i])) {
			  			...
			  			blocks_Sprite[i].center = {x:chipmunk.cpBodyGetPos(blocks_Body[i]).x,y:cpY(chipmunk.cpBodyGetPos(blocks_Body[i]).y)};
		    ...
		    // C) THE BOMB HAS ALREADY EXPLODED > (bombExploded === true)
		    	...
		    	while(i--){
		    		...
		    		var velocity = Math.floor(chipmunk.cpBodyGetVel(blocks_Body[i]).y);  	  //Y VELOCITY OF THE BODY
			    	var velAbs = Math.abs(velocity);										  //ABSOLUTE VALUE OF THE BODY		    	
	...
	function onTouchMoveBomb(e){
		...
		if(bombTouched){
			...
			var touch = {x:bomb_Sprite.center.x - e.x*game.touchScaleX, y: bomb_Sprite.center.y - e.y*game.touchScaleY};	
